...
% Loading the image
I = imread('retina2.jpg');
img(:,:) = I(:,:,2); % Take the green channel only
%img(:,:) = rgb2gray(I);

% Visualize the loaded image
figure; imshow(I); title('A retinal image');

k = fspecial('gaussian',5,2); % Initializing the gaussian kernel
...
gx=imfilter(img, kx); % x gradient
gy=imfilter(img, ky); % y gradient
...
for i=1:n
    [r, c]=find(L==i);
    [rn, r2]=size(r);
    if rn<10                      % the threshold of length filtering 
