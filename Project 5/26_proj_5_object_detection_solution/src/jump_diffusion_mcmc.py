...
def diffusion(t, path_im, I, T, theta, K, lik):
    ...
    for ki in range(K):
        ...
        plt.savefig(src1+"/figs/{}/{}_{}.png".format(path_im, t, ki))
...
def jump_diffusion_mcmc(path_im, path_target, k):
    in_img = cv2.imread(src1+"/discs{}.bmp".format(path_im))
    in_tar = cv2.imread(src1+"/{}".format(path_target))
    I = cv2.cvtColor(in_img, cv2.COLOR_RGB2GRAY)/255.
    T = cv2.cvtColor(in_tar, cv2.COLOR_RGB2GRAY)/255.

    #in_img = cv2.imread("D:\PhD\OneDrive - Oklahoma A and M System\CM\Project 5\discs3_small.bmp")
    ...
    Io = drawcircle(I, theta, k)
    plt.imshow(Io, cmap='gray')
    plt.savefig("../figs/{}_initial.png".format(path_im))
...
src1 = "D:/PhD/OneDrive - Oklahoma A and M System/CM/Project 5/26_proj_5_object_detection_solution/project5"
if __name__ == '__main__':
    for im in [1, 2, 3]:
        #os.system("rm ../figs/discs{}/*.png".format(im))
        jump_diffusion_mcmc(im, "target.bmp", k=5)
...
#in_img = cv2.imread("D:/PhD/OneDrive - Oklahoma A and M System/CM/Project 5/26_proj_5_object_detection_solution/project5/discs{}.bmp".format(2))
