...
src = "D:/PhD/OneDrive - Oklahoma A and M System/CM/Project 5/py"

out = cv2.VideoWriter(src+'/figs/outpy.avi', cv2.VideoWriter_fourcc(*'MJPG'), 1, (128, 128))
...
def diffusion(t, path_im, I, T, theta, K, lik):
    ...
    for ki in range(K):
        ...
        try:
            os.mkdir(src+'/figs/'+path_im[:-4])
        except:
            pass
        plt.savefig(src+"/figs/{}/{:03d}_{:02d}.png".format(path_im[:-4], t, ki))
...
def jump_diffusion_mcmc(path_im, path_target, k):
    in_img = cv2.imread(src+"/project5/{}".format(path_im))
    in_tar = cv2.imread(src+"/project5/{}".format(path_target))
    ...
    Io = drawcircle(I, theta, k)
    plt.imshow(Io, cmap='gray')
    plt.savefig(src+"/figs/{}_initial.png".format(path_im[:-4]))
    ...
    plt.title("Final")
    plt.imshow(Io, cmap='gray')
    plt.savefig(src+"/figs/{}_final.png".format(path_im[:-4]))


if __name__ == '__main__':
    for im in [4]:
        os.system(src+"/figs/discs{}/*.png".format(im))
